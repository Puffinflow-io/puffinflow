...
class Agents:
    ...
    def then_add(self, agent: Agent) -> 'Agents':
        ...
        if self._current_stage_agents:
            self._stages.append({
                'type': 'sequential',
                'agents': self._current_stage_agents.copy()
            })
    ...
    def if_output(self, agent_name: str, key: str, expected_value: Any) -> 'ConditionalAgents':
    ...
    async def get_aggregated_result(self) -> Dict[str, Any]:
        ...
        if self._aggregator:
            ...
            return {
                ...
                'total_duration': main_result.total_duration + aggregator_result.execution_duration
            }
...
class PipelineAgents:
    ...
    def pipe_output(self, from_agent: str, output_key: str,
                    to_agent: str, input_key: str) -> 'PipelineAgents':
            ...
            if i > 0:
                ...
                if prev_result:
                    ...
                    for pipe_config in self._pipe_configs:
                        if (pipe_config['from'] == prev_agent.name and
                                pipe_config['to'] == agent.name):

                            output_value = prev_result.get_output(pipe_config['output_key'])
                            if output_value is not None:
                                agent.set_variable(pipe_config['input_key'], output_value)
...
class FluentResult:
    ...
    def filter_successful(self) -> List[AgentResult]:
        ...
        if hasattr(self._team_result, 'agent_results'):
            return [
                result for result in self._team_result.agent_results.values()
...
async def run_parallel_agents(*agents: Agent, timeout: Optional[float] = None) -> FluentResult:
...
async def get_best_agent(agents: List[Agent], metric: str, maximize: bool = True) -> AgentResult:
