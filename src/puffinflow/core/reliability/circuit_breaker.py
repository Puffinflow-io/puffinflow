...
class CircuitBreaker:
    ...
    async def protect(self) -> AsyncGenerator[None, None]:
        ...
        # Fast path: when CLOSED with no failures, skip the lock entirely
        if self.state == CircuitState.CLOSED and self._failure_count == 0:
            try:
                yield
                # No need to record success when fully healthy
            except Exception:
                await self._record_failure()
                raise
            return

        # Full path (OPEN, HALF_OPEN, or CLOSED with prior failures)
