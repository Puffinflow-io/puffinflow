...
class FlexibleStateDecorator:
    ...
    # Shared empty config for bare @state — avoids per-function dict allocation
    _BARE_CONFIG: dict[str, Any] = {}

    def _decorate_bare(self, func: Callable) -> Callable:
        """Ultra-fast decoration for bare @state / @state() with no config."""
        func._puffinflow_state = True  # type: ignore
        func._state_name = func.__name__  # type: ignore
        func._priority = Priority.NORMAL  # type: ignore
        func._state_config = FlexibleStateDecorator._BARE_CONFIG  # type: ignore
        return func
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Callable[..., Any]:
        ...
        if len(args) == 1 and callable(args[0]) and not kwargs:
            ...
            if not self.default_config:
                return self._decorate_bare(func)
    ...
    # Resource-related config keys
    _RESOURCE_KEYS = frozenset({"cpu", "memory", "io", "network", "gpu", "timeout"})

    def _decorate_function(self, func: Callable, config: dict[str, Any]) -> Callable:
        """Apply decoration with merged configuration."""
        # Track whether user explicitly set any resource-related keys
        has_explicit_resources = bool(config.keys() & self._RESOURCE_KEYS)
        ...
        config["_has_explicit_resources"] = has_explicit_resources
    ...
    def _apply_configuration(self, func: Callable, config: dict[str, Any]) -> Callable:
        ...
        has_explicit_resources = config.pop("_has_explicit_resources", True)

        # Only create resource requirements if user explicitly set resource args
        # (or if a profile was applied). This allows bare @state() to use the
        # fast execution path with zero resource overhead.
        ...
        # CRITICAL: Mark as PuffinFlow state — these are always needed
        func._puffinflow_state = True  # type: ignore
        func._state_name = func.__name__  # type: ignore
        func._priority = config["priority"]  # type: ignore
        func._state_config = config  # type: ignore

        # Only set attributes that are actually used by add_state() or features
        if requirements is not None:
            func._resource_requirements = requirements  # type: ignore

        # Only set non-default/non-empty attributes to minimize per-function overhead
        tags = config.get("tags")
        if tags:
            func._state_tags = tags  # type: ignore

        desc = config.get("description", "")
        if desc:
            func._state_description = desc  # type: ignore

        func._dependency_configs = dependency_configs  # type: ignore

        if coordination_primitive is not None:
