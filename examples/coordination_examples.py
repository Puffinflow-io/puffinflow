...
class DataAggregator(Agent):
    ...
    async def aggregate_results(self, context: Context):
        ...
        print(f"{self.name} aggregated data from {len(aggregated_result['sources'])} sources")
        return None
...
async def run_mixed_coordination():
    ...
    collectors = [
        DataCollector(f"mixed-collector-{i}") for i in range(3)
    ]
    ...
    # Summary
    total_agents = len(collection_results) + len(processing_results) + len(final_results)
...
class MessagePassingAgent(Agent):
    ...
    def __init__(self, name: str, message_target: str = None):
        super().__init__(name)
    ...
    def receive_message(self, message: dict):
        ...
        print(f"{self.name} received message from {message['from']}: {message['content']}")
