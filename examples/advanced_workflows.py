...
from puffinflow import (
    Agent, Context, AgentTeam, state, cpu_intensive, memory_intensive,
...
class WorkflowOrchestrator(Agent):
    ...
    def __init__(self, name: str, workflow_config: Dict[str, Any] = None):
        super().__init__(name)
    ...
    async def plan_execution(self, context: Context):
        ...
        execution_plans = {
            "data_processing": {
                "stages": [
                    ...
                    {"name": "data_transformation", "parallel": True, "critical": False},
        ...
        execution_plan = execution_plans.get(workflow_type, execution_plans["data_processing"])
        ...
        for stage in stages:
            ...
            try:
                ...
                if stage.get("critical", False):
                    print(f"    Critical stage {stage_name} failed - initiating recovery")
                    recovery_result = await self.handle_stage_failure(stage_name, str(e))
                    if recovery_result:
                        completed_stages.add(stage_name)
    ...
    async def finalize_workflow(self, context: Context):
        ...
        print(f"  Efficiency: {summary['performance_metrics']['efficiency_score']:.1f}%")
    ...
    def calculate_avg_stage_time(self, workflow_result: Dict[str, Any]) -> float:
        ...
        total_time = sum(result.get("execution_time", 0) for result in stage_results.values())
    ...
    def generate_recommendations(self, workflow_result: Dict[str, Any]) -> List[str]:
        ...
        if workflow_result.get("stages_failed", 0) > 0:
            recommendations.append("Review failed stages and implement preventive measures")
...
class DynamicWorkflowGenerator(Agent):
    ...
    async def analyze_requirements(self, context: Context):
        ...
        print(f"{self.name} generated workflow with {len(workflow_spec['stages'])} stages")
    ...
    def generate_workflow_spec(self, requirements: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "resource_requirements": self.calculate_resource_requirements(stages, requirements),
            "parallelization_opportunities": self.identify_parallelization(stages)
        }
    
    def calculate_resource_requirements(self, stages: List[str], requirements: Dict[str, Any]) -> Dict[str, float]:
        ...
        return {
            "cpu": min(base_cpu * cpu_multiplier * volume_mult, requirements["resource_constraints"]["max_cpu"]),
            "memory": min(base_memory * memory_multiplier * volume_mult, requirements["resource_constraints"]["max_memory"])
        }
    ...
    def identify_parallelization(self, stages: List[str]) -> List[str]:
        ...
        parallel_candidates = ["parallel_processing", "advanced_analysis", "quality_assurance"]
    ...
    async def validate_workflow(self, context: Context):
        ...
        validation_results = {
            "resource_feasibility": self.check_resource_feasibility(workflow_spec, requirements),
            "time_feasibility": self.check_time_feasibility(workflow_spec, requirements),
            "quality_feasibility": self.check_quality_feasibility(workflow_spec, requirements),
            "overall_score": 0.0
        }
        ...
        if validation_results["overall_score"] > 0.8:
            print(f"{self.name} workflow validation passed: {validation_results['overall_score']:.2f}")
            return "execute_dynamic_workflow"
        else:
            print(f"{self.name} workflow validation failed: {validation_results['overall_score']:.2f}")
            return "optimize_workflow"
    
    def check_resource_feasibility(self, workflow_spec: Dict[str, Any], requirements: Dict[str, Any]) -> float:
    ...
    def check_time_feasibility(self, workflow_spec: Dict[str, Any], requirements: Dict[str, Any]) -> float:
    ...
    def check_quality_feasibility(self, workflow_spec: Dict[str, Any], requirements: Dict[str, Any]) -> float:
    ...
    async def optimize_workflow(self, context: Context):
        ...
        if validation_results.get("resource_feasibility", 1.0) < 0.8:
            ...
            print(f"  Applied resource optimization")
        
        if validation_results.get("time_feasibility", 1.0) < 0.8:
            # Add parallelization
            optimized_spec["parallelization_opportunities"].extend(["input_validation", "output_generation"])
            optimized_spec["estimated_duration"] *= 0.7
            print(f"  Applied time optimization")
    ...
    async def execute_dynamic_workflow(self, context: Context):
        ...
        workflow_spec = context.get_output("optimized_workflow_spec") or context.get_output("workflow_spec", {})
        ...
        for stage in stages:
            ...
            print(f"  Executed {stage} ({'parallel' if stage in parallelizable else 'sequential'})")
        ...
        dynamic_result = {
            ...
            "sequential_stages": len([s for s in execution_results if not s["parallel"]]),
        ...
        context.set_metric("dynamic_workflow_efficiency", dynamic_result["efficiency_score"])
...
async def demonstrate_conditional_workflows():
    ...
    conditional_agents = [ConditionalWorkflowAgent(f"conditional-agent-{i}") for i in range(3)]
    ...
    for agent in conditional_agents:
        ...
        print(f"  {agent.name}: {execution_path} "
...
async def demonstrate_dynamic_workflows():
    ...
    for generator in generators:
        ...
        print(f"    Execution Time: {dynamic_result.get('total_execution_time', 0):.2f}s")
        print(f"    Efficiency: {dynamic_result.get('efficiency_score', 0):.1f} stages/sec")
...
async def run_comprehensive_workflow_test():
    ...
    print(f"Success rate: {successful_agents / len(team_result.agent_results) * 100:.1f}%")
    print()
