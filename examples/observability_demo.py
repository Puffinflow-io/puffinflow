...
from puffinflow import (
    Agent, Context, state, cpu_intensive, memory_intensive
)
from puffinflow.core.observability import (
    ObservabilityConfig, setup_observability, get_observability,
    observe, trace_state, ObservableAgent, ObservableContext
)
...
class MonitoredAgent(Agent):
    ...
    async def start_monitoring(self, context: Context):
        ...
        avg_cpu = sum(s["cpu_usage"] for s in performance_samples) / len(performance_samples)
        avg_memory = sum(s["memory_usage"] for s in performance_samples) / len(performance_samples)
    ...
    async def analyze_performance(self, context: Context):
        ...
        analysis = {
            "cpu_variance": monitoring_result.get("average_cpu", 0) - baseline_metrics.get("cpu_baseline", 0),
            "memory_variance": monitoring_result.get("average_memory", 0) - baseline_metrics.get("memory_baseline", 0),
        ...
        print(f"{self.name} performance analysis: {analysis['performance_score']:.2f} score")
        return None
    ...
    async def process_data_pipeline(self, context: Context):
        ...
        with self.create_trace_span("pipeline_execution") as pipeline_span:
            ...
            with self.create_trace_span("data_validation") as validation_span:
                ...
                validation_span.set_attribute("validation_success", validation_errors == 0)
            
            # Stage 3: Data transformation
            with self.create_trace_span("data_transformation") as transform_span:
                await asyncio.sleep(0.15)
                transform_span.set_attribute("transformation_type", "normalize_and_enrich")
...
class AlertingAgent(Agent):
    ...
    async def monitor_system_health(self, context: Context):
        ...
        if alerts_triggered:
            ...
            for alert in alerts_triggered:
                print(f"  ALERT: {alert['metric']} = {alert['value']:.1f} "
    ...
    async def generate_health_report(self, context: Context):
        ...
        print(f"{self.name} health report: {health_score:.1f}/100 ({health_report['health_status']})")
        return None
    ...
    def generate_recommendations(self, alerts: list) -> list:
        ...
        for alert in alerts:
            metric = alert["metric"]
            if metric == "cpu_usage":
                recommendations.append("Consider scaling CPU resources or optimizing workloads")
                ...
                recommendations.append("Optimize performance bottlenecks and consider caching")
...
async def demonstrate_basic_observability():
    ...
    for metric_name in ["cpu_usage", "memory_usage", "initialization_time"]:
        ...
        if summary:
            print(f"  {metric_name}: avg={summary['avg']:.2f}, "
...
async def run_observability_comparison():
    ...
    overhead = ((monitored_time - simple_time) / simple_time) * 100 if simple_time > 0 else 0
