...
from puffinflow import (
    Agent, Context, CircuitBreaker, CircuitBreakerConfig, Bulkhead, BulkheadConfig,
    ResourceLeakDetector, state, external_service, fault_tolerant
)
    ...
    async def call_external_api(self, context: Context):
        ...
        async def api_call():
            ...
            if random.random() > reliability:
                raise Exception(f"External service failure (reliability: {reliability})")
...
class ResourceIntensiveAgent(Agent):
    ...
    async def cleanup_resources(self, context: Context):
        ...
        if leak_report.get("potential_leaks", 0) > 0:
            print(f"{self.name} detected {leak_report['potential_leaks']} potential resource leaks")
            context.set_metric("resource_leaks_detected", leak_report["potential_leaks"])
