...
from puffinflow import (
    Agent, Context, ResourcePool, ResourceRequirements, ResourceType,
    QuotaManager, AllocationStrategy, state, cpu_intensive, memory_intensive
)
...
class ResourceAwareAgent(Agent):
    ...
    async def execute_workload(self, context: Context):
        ...
        context.set_output("execution_result", {
            "workload": self.get_variable("workload_size"),
            "planned_duration": duration,
            "actual_duration": actual_duration,
            "efficiency": duration / actual_duration if actual_duration > 0 else 1.0
        })
...
class DatabaseAgent(Agent):
    ...
    async def query_database(self, context: Context):
        ...
        context.set_metric("rows_per_second", result["rows_processed"] / result["execution_time"])
...
async def demonstrate_resource_pool():
    ...
    small_req = ResourceRequirements(
        cpu=2.0,
        memory=1.0,
        priority="normal"
    )
    
    large_req = ResourceRequirements(
        cpu=8.0,
        memory=16.0,
        priority="high"
    )
...
async def demonstrate_quota_management():
    ...
    print("Set quotas:")
    print(f"  Team A: {quota_manager.get_quota('team_a', ResourceType.CPU)} CPU, "
          f"{quota_manager.get_quota('team_a', ResourceType.MEMORY)} GB RAM")
    print(f"  Team B: {quota_manager.get_quota('team_b', ResourceType.CPU)} CPU, "
...
async def demonstrate_allocation_strategies():
    ...
    for i, req in enumerate(requests):
        print(f"  Request {i+1}: {req.cpu} CPU, {req.memory}GB RAM, priority: {req.priority}")
