...
from puffinflow import Agent, state, Priority, ExecutionMode
...
class TestReliabilityExamples:
    ...
    async def test_data_consistency_validation(self):
        ...
        async def create_data_snapshot(context):
            ...
            critical_data = {
                "user_sessions": {"session_1": {"user_id": "user_123", "created_at": time.time()}},
                "processing_queue": [{"id": "item_1", "type": "process", "status": "pending"}],
                "cache_state": {"key_1": {"data": "value_1", "expires_at": time.time() + 3600}},
                "configuration": {"service_level": "full", "feature_flags": {"feature_a": True}}
            }
            ...
            for component, data in snapshot["data"].items():
                try:
                    if component == "user_sessions":
                        ...
                        valid = isinstance(data, dict) and all(
                            isinstance(value, dict) and "data" in value and "expires_at" in value
                        ...
                        valid = isinstance(data, dict) and "service_level" in data and "feature_flags" in data
                        validation_results[component] = {
                            "valid": valid,
        ...
        result = await validation_agent.run(
            initial_context={"data_snapshot": snapshot}
        )
