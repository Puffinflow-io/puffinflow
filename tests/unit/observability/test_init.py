...
class TestObservabilityModuleImports:
    ...
    def test_no_unexpected_exports(self):
        ...
        public_attrs = [attr for attr in dir(puffinflow.core.observability) 
...
class TestObservabilityModuleStructure:
    ...
    def test_exports_are_logically_grouped(self):
        ...
        core_items = [item for item in __all__ if any(keyword in item.lower() 
                     for keyword in ['manager', 'get_', 'setup_'])]
        config_items = [item for item in __all__ if 'config' in item.lower()]
        decorator_items = [item for item in __all__ if any(keyword in item.lower() 
                          for keyword in ['observe', 'trace'])]
