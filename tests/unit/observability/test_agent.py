...
class TestObservableAgent:
    ...
    def test_observable_agent_creation(self):
        ...
        with patch('src.puffinflow.core.observability.agent.Agent.__init__') as mock_super_init:
    ...
    def test_create_context(self):
        ...
        with patch('src.puffinflow.core.observability.agent.Agent.__init__'):
            agent = ObservableAgent("test-agent", observability=mock_observability, workflow_id="wf-123")
    ...
    async def test_run_with_tracing(self):
        ...
        with patch('src.puffinflow.core.observability.agent.Agent.__init__'):
            with patch('src.puffinflow.core.observability.agent.Agent.run', new_callable=AsyncMock) as mock_super_run:
                agent = ObservableAgent("test-agent", observability=mock_observability, workflow_id="wf-123")
                agent.name = "test-agent"  # Set name since we're mocking parent __init__
    ...
    async def test_run_with_tracing_exception(self):
        ...
        with patch('src.puffinflow.core.observability.agent.Agent.__init__'):
            with patch('src.puffinflow.core.observability.agent.Agent.run', new_callable=AsyncMock, side_effect=test_exception) as mock_super_run:
                agent = ObservableAgent("test-agent", observability=mock_observability)
                agent.name = "test-agent"  # Set name since we're mocking parent __init__
                agent.workflow_duration = mock_histogram
    ...
    async def test_run_state_with_tracing(self):
        ...
        with patch('src.puffinflow.core.observability.agent.Agent.__init__'):
            ...
            with patch.object(agent, '_create_context') as mock_create_context:
                ...
                mock_context.set_variable.assert_called_once_with("current_state", "test_state")
    ...
    async def test_run_state_without_tracing(self):
        ...
        with patch('src.puffinflow.core.observability.agent.Agent.__init__'):
            with patch('src.puffinflow.core.observability.agent.Agent.run_state', new_callable=AsyncMock) as mock_super_run_state:
                agent = ObservableAgent("test-agent", observability=mock_observability)
