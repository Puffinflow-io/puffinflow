...
class TestStateManagement:
    ...
    def test_add_state_basic(self, agent, simple_state_func):
        ...
        assert metadata.resources is None  # Resources are lazily created on demand
...
class TestQueueManagement:
    ...
    async def test_can_run_basic_state(self, agent, simple_state_func):
        ...
        can_run = agent._can_run("test_state")
...
class TestWorkflowExecution:
    ...
    async def test_timeout_mechanism(self, agent):
        ...
        with patch("time.time") as mock_time:
            # First call sets start_time in run(), second call is the
...
class TestEdgeCases:
    ...
    async def test_circular_dependencies(self, agent):
        ...
        assert not agent._can_run("state1")
        assert not agent._can_run("state2")
    ...
    async def test_agent_execution_metadata_and_metrics(self, agent, simple_state_func):
        ...
        # resource_usage, circuit_breaker_metrics, bulkhead_metrics only
        # present when those objects have been lazily initialized
        assert "resource_usage" not in metrics  # not yet initialized
        assert "circuit_breaker_metrics" not in metrics
        assert "bulkhead_metrics" not in metrics
