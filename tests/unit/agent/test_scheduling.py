...
from src.puffinflow.core.agent.scheduling.scheduler import GlobalScheduler, ScheduledAgent, ScheduledJob
from src.puffinflow.core.agent.scheduling.parser import ScheduleParser, parse_schedule_string
from src.puffinflow.core.agent.scheduling.inputs import parse_magic_prefix, InputType, parse_inputs
...
class TestAgentScheduling:
    ...
    def test_agent_schedule_method(self):
        ...
        with patch('src.puffinflow.core.agent.scheduling.scheduler.GlobalScheduler.get_instance_sync') as mock_scheduler:
            mock_scheduler_instance = Mock()
            mock_scheduler.return_value = mock_scheduler_instance
            mock_scheduler_instance.schedule_agent.return_value = Mock(spec=ScheduledAgent)
            
            result = agent.schedule("daily", source="database", api_key="secret:sk-123")
    ...
    def test_agent_every_method(self):
        ...
        assert result._schedule_string == "every 2 hours"  # Should handle this gracefully
...
class TestScheduledAgent:
    ...
    def test_scheduled_agent_cancel(self):
        ...
        with patch('src.puffinflow.core.agent.scheduling.scheduler.GlobalScheduler.get_instance_sync') as mock_get_instance:
            mock_scheduler = Mock()
