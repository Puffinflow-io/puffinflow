...
from puffinflow.core.agent.decorators.flexible import (
    StateProfile, PROFILES, FlexibleStateDecorator, state,
    ...
    fault_tolerant, external_service, high_availability,
...
class TestPredefinedProfiles:
    ...
    def test_profiles_exist(self):
        ...
        expected_profiles = [
            ...
            "resilient", "critical_no_dlq", "fault_tolerant",
            "external_service", "high_availability"
        ]
    ...
    def test_io_intensive_profile(self):
        ...
        assert profile.circuit_breaker is True
        assert profile.circuit_breaker_config == {"failure_threshold": 3, "recovery_timeout": 30.0}
        assert profile.bulkhead is True
        assert profile.bulkhead_config == {"max_concurrent": 5}
    ...
    def test_fault_tolerant_profile(self):
        ...
        assert profile.max_retries == 5
        assert profile.circuit_breaker is True
        assert profile.circuit_breaker_config == {"failure_threshold": 3, "recovery_timeout": 45.0}
        assert profile.bulkhead is True
