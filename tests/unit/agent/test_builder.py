...
class TestStateBuilder:
    ...
    def test_mutex_configuration(self):
        ...
        assert result is builder
        assert builder._config['mutex'] == True
    ...
    def test_depends_on_multiple(self):
        ...
        assert result is builder
        assert builder._config['depends_on'] == ['init_state', 'setup_state', 'validate_state']
    ...
    def test_retry_configuration_full(self):
        ...
        assert builder._config['retry_config']['jitter'] == False
        assert builder._config['retry_config']['dead_letter_on_max_retries'] == False
        assert builder._config['retry_config']['dead_letter_on_timeout'] == False
        assert builder._config['circuit_breaker'] == True
    ...
    def test_retry_configuration_minimal(self):
        ...
        assert builder._config['retry_config']['jitter'] == True  # default
    ...
    def test_enable_dead_letter(self):
        ...
        assert result is builder
        assert builder._config['dead_letter'] == True
    ...
    def test_disable_dead_letter(self):
        ...
        assert result is builder
        assert builder._config['no_dead_letter'] == True
    ...
    def test_with_dead_letter(self):
        ...
        assert result is builder
        assert builder._config['dead_letter'] == False
    ...
    def test_circuit_breaker_disabled(self):
        ...
        assert result is builder
        assert builder._config['circuit_breaker'] == False
    
    def test_with_circuit_breaker(self):
        """Test circuit breaker with custom config."""
        builder = StateBuilder()
        result = builder.with_circuit_breaker(failure_threshold=3, recovery_timeout=30.0)
        assert result is builder
        assert builder._config['circuit_breaker'] == True
    ...
    def test_bulkhead_full_config(self):
        ...
        assert result is builder
        assert builder._config['bulkhead'] == True
    ...
    def test_bulkhead_disabled(self):
        ...
        assert result is builder
        assert builder._config['bulkhead'] == False
    ...
    def test_leak_detection(self):
        ...
        assert result is builder
        assert builder._config['leak_detection'] == True
        
        result = builder.leak_detection(False)
        assert result is builder
        assert builder._config['leak_detection'] == False
    ...
    def test_fault_tolerant(self):
        ...
        assert builder._config['retry_config']['max_retries'] == 5
        assert builder._config['retry_config']['dead_letter_on_max_retries'] == True
    ...
    def test_fault_tolerant_partial(self):
        ...
        assert result is builder
        assert 'circuit_breaker' not in builder._config or not builder._config['circuit_breaker']
        assert builder._config['bulkhead'] == True
    ...
    def test_preemptible(self):
        ...
        assert result is builder
        assert builder._config['preemptible'] == True
        
        result = builder.preemptible(False)
        assert result is builder
        assert builder._config['preemptible'] == False
    ...
    def test_cleanup_on_failure(self):
        ...
        assert result is builder
        assert builder._config['cleanup_on_failure'] == True
        
        result = builder.cleanup_on_failure(False)
        assert result is builder
        assert builder._config['cleanup_on_failure'] == False
