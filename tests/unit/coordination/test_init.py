...
class TestCoordinationModuleImports:
    ...
    def test_all_exports_defined(self):
        ...
        expected_exports = [
            # Team coordination
            "AgentTeam", "TeamResult", "EventBus", "Message", "Event",
            "create_team", "run_agents_parallel", "run_agents_sequential",
            ...
            # Agent pools
            "AgentPool", "WorkQueue", "WorkItem", "CompletedWork",
            "DynamicProcessingPool", "ScalingPolicy", "PoolContext", "WorkProcessor",
            
            # Existing coordination (if available)
            "AgentCoordinator", "CoordinationConfig", "enhance_agent",
    ...
    def test_no_unexpected_exports(self):
        ...
        public_attrs = [attr for attr in dir(puffinflow.core.coordination)
                       if not attr.startswith('_')]
        
        # Filter out imported submodules which are implementation details
        submodules = ['agent_group', 'agent_pool', 'agent_team', 'coordinator',
                     'deadlock', 'fluent_api', 'primitives', 'rate_limiter']
...
class TestCoordinationModuleStructure:
    ...
    def test_exports_are_sorted_by_category(self):
        ...
        team_items = [item for item in __all__ if any(keyword in item.lower() 
                     for keyword in ['team', 'event', 'message', 'create_team', 'run_agents'])]
        group_items = [item for item in __all__ if any(keyword in item.lower() 
