...
from src.puffinflow.core.coordination.agent_pool import (
    AgentPool,
    ScalingPolicy,
    WorkItem,
    CompletedWork,
...
class TestDynamicProcessingPool:
    ...
    async def test_process_workload(self):
        ...
        with patch.object(pool.pool, 'auto_scale', return_value=mock_context):
            ...
            with patch.object(pool.pool, 'process_queue', side_effect=mock_process_queue):
...
class TestWorkQueueAdvanced:
    ...
    def test_work_queue_batch_operations(self):
        ...
        # Add multiple items
        work_items = [
            WorkItem(id=f"test_{i}", data={"task": f"test{i}"})
            for i in range(5)
        ]
...
class TestAgentPoolCoverageEnhancement:
    ...
    async def test_pool_context_cleanup_with_scaling(self):
        ...
        assert pool._scaling_task is None or pool._scaling_task.cancelled() or pool._scaling_task.done()
    ...
    async def test_dynamic_processing_pool_comprehensive(self):
        ...
        work_items = [
            WorkItem(id=f"work_{i}", data=f"data_{i}")
            for i in range(3)
        ]
