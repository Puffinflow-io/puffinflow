...
class TestAgentCoordination:
    ...
    async def test_parallel_agent_execution(self):
        ...
        collectors = [
            TestDataCollector(f"collector-{i}", data_size=50 + i * 10)
            for i in range(3)
        ]
    ...
    async def test_sequential_agent_execution(self):
        ...
        assert execution_time >= 0.4  # At least 0.1 + 0.24 + 0.15 = 0.49s
    ...
    async def test_agent_team_coordination(self):
        ...
        assert processor1_result.get_output("processed_data")["processing_factor"] == 0.8
        assert processor2_result.get_output("processed_data")["processing_factor"] == 1.5
    ...
    async def test_mixed_coordination_patterns(self):
        ...
        collectors = [
            TestDataCollector(f"mixed-collector-{i}", data_size=30)
            for i in range(2)
        ]
        ...
        # Handle both string and enum status
        status = aggregation_result.status.name if hasattr(aggregation_result.status, 'name') else str(aggregation_result.status)
        assert status.upper() in ["COMPLETED", "SUCCESS"]
        ...
        proc1_data = processing_results["mixed-processor-1"].get_output("processed_data")
        proc2_data = processing_results["mixed-processor-2"].get_output("processed_data")
    ...
    async def test_agent_failure_handling(self):
        ...
        success_count = sum(
            ...
            if (result.status.name if hasattr(result.status, 'name') else str(result.status)).upper() in ["COMPLETED", "SUCCESS"]
        )
        failure_count = sum(
            1 for result in results.values()
            if (result.status.name if hasattr(result.status, 'name') else str(result.status)).upper() in ["FAILED", "ERROR"]
        )
    ...
    async def test_resource_coordination(self):
        ...
        total_memory = sum(result.get_output("memory_used", 0) for result in results.values())
    ...
    async def test_context_sharing_between_agents(self):
        ...
        shared_values = [result.get_output("shared_value") for result in results.values()]
...
class TestAdvancedCoordination:
    ...
    async def test_conditional_coordination(self):
        ...
        next_actions = {
            name: result.get_output("next_action")
            for name, result in results.items()
        }
    ...
    async def test_cascading_coordination(self):
        ...
        trigger_count = sum(
            1 for result in trigger_results.values()
            if result.get_output("should_trigger", False)
        )
