...
class DataStorageAgent(Agent):
    ...
    async def store_data(self, context: Context):
        ...
        transformed_data = {
            "records": [{"id": i, "value": f"data_{i}", "transformed": True} for i in range(800)],
            "metadata": {"transformation_type": "filter", "input_count": 1000, "output_count": 800}
        }
...
class TestRealWorldScenarios:
    ...
    async def test_batch_processing_scenario(self):
        ...
        ingestion_agent = DataIngestionAgent("batch-ingester", {
            "url": "https://data.warehouse.com/export",
            "format": "csv",
            "expected_records": 5000
        })
        
        transformation_agent = DataTransformationAgent("batch-transformer", {
            "type": "filter",
            "multiplier": 0.9  # 90% pass rate
        })
        
        storage_agent = DataStorageAgent("batch-storage", {
            "type": "warehouse",
            "batch_size": 500
        })
        ...
        assert transformed == 900  # Transformation agent filtered to 900 records (90% of 1000)
    ...
    async def test_microservices_orchestration(self):
        ...
        auth_service = APIServiceAgent("auth-service", {
            "endpoint": "https://auth.service.com/validate",
            "timeout": 10
        })
        
        data_service = APIServiceAgent("data-service", {
            "endpoint": "https://data.service.com/fetch",
            "timeout": 30
        })
        
        notification_service = APIServiceAgent("notification-service", {
            "endpoint": "https://notify.service.com/send",
            "timeout": 15
        })
