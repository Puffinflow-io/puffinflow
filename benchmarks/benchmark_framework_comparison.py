...
class PuffinFlowBenchmarks:
    ...
    def coordination_benchmark(self, context=None):
        ...
        try:
            ...
            result = loop.run_until_complete(asyncio.sleep(0.001))
...
class DagsterBenchmarks:
    ...
    def setup_simple_workflow(self):
        ...
        try:
            from dagster import asset, materialize, DagsterInstance
    ...
    def simple_task_execution(self, context=None):
        ...
        try:
            ...
            result = context["materialize"]([context["simple_asset"]], instance=context["instance"])
    ...
    def coordination_benchmark(self, context=None):
        ...
        try:
            ...
            assets = [context["data_asset"], context["processed_asset"]]
...
def run_framework_comparison_benchmarks():
    ...
    benchmarks = [
        ...
        # Multi-Task Workflow
        {
            ...
            "frameworks": [
                ("PuffinFlow", puffinflow_bench.multi_task_workflow, 
                 puffinflow_bench.setup_simple_workflow, 
                 puffinflow_bench.teardown_simple_workflow),
                ("Dagster", dagster_bench.multi_task_workflow, 
                 dagster_bench.setup_simple_workflow, 
                 dagster_bench.teardown_simple_workflow),
                ("Prefect", prefect_bench.multi_task_workflow, 
                 prefect_bench.setup_simple_workflow, 
                 prefect_bench.teardown_simple_workflow),
                ("LangGraph", langgraph_bench.multi_task_workflow, 
                 langgraph_bench.setup_simple_workflow, 
                 langgraph_bench.teardown_simple_workflow),
            ]
        },
        # Coordination Benchmark
        {
            ...
            "frameworks": [
                ("PuffinFlow", puffinflow_bench.coordination_benchmark, 
                 puffinflow_bench.setup_simple_workflow, 
                 puffinflow_bench.teardown_simple_workflow),
                ("Dagster", dagster_bench.coordination_benchmark, 
                 dagster_bench.setup_simple_workflow, 
                 dagster_bench.teardown_simple_workflow),
                ("Prefect", prefect_bench.coordination_benchmark, 
                 prefect_bench.setup_simple_workflow, 
                 prefect_bench.teardown_simple_workflow),
                ("LangGraph", langgraph_bench.coordination_benchmark, 
                 langgraph_bench.setup_simple_workflow, 
                 langgraph_bench.teardown_simple_workflow),
            ]
        }
    ...
    for benchmark in benchmarks:
        ...
        for framework_name, bench_func, setup_func, teardown_func in benchmark["frameworks"]:
            try:
...
def print_comparison_results(results: list[FrameworkBenchmarkResult]):
    ...
    for benchmark_name, benchmark_results in benchmark_groups.items():
        ...
        print(f"{'Framework':<12} {'Avg (ms)':<10} {'Throughput':<12} {'Memory (MB)':<12} {'Setup (ms)':<12}")
        print("-" * 80)
        
        for i, result in enumerate(sorted_results):
            rank_symbol = "🥇" if i == 0 else "🥈" if i == 1 else "🥉" if i == 2 else "  "
            print(f"{rank_symbol} {result.framework:<10} {result.duration_ms:<10.2f} "
    ...
    for benchmark_name, benchmark_results in benchmark_groups.items():
        ...
        for i, result in enumerate(sorted_results):
            ...
            framework_scores[result.framework].append(len(sorted_results) - i)  # Higher score is better
    ...
    ranked_frameworks = sorted(framework_avg_scores.items(), key=lambda x: x[1], reverse=True)
    ...
    for i, (framework, avg_score) in enumerate(ranked_frameworks):
        ...
        avg_duration = sum(r.duration_ms for r in framework_results) / len(framework_results)
        avg_throughput = sum(r.throughput_ops_per_sec for r in framework_results) / len(framework_results)
        
        profile = "Fast" if avg_duration < 10 else "Medium" if avg_duration < 50 else "Slow"
        
        print(f"{rank_symbol:<6} {framework:<12} {avg_score:<12.1f} {profile} ({avg_duration:.1f}ms, {avg_throughput:.1f} ops/s)")
