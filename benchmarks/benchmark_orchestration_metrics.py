...
class OrchestrationBenchmarkRunner:
    ...
    def _aggregate_metrics(self, all_metrics: List[List[OrchestrationMetric]]) -> List[OrchestrationMetric]:
        ...
        for name, values in metric_groups.items():
            ...
            if template_metric and values:
                ...
                aggregated.append(OrchestrationMetric(
                    name=name,
                    value=avg_value,
...
class WorkflowComplexityBenchmarks:
    ...
    def deep_dependency_chain(self) -> List[OrchestrationMetric]:
        ...
        class ChainAgent(Agent):
            def __init__(self, agent_id: int, depends_on: Optional['ChainAgent'] = None):
            ...
            async def run(self):
                ...
                self.result = {"agent_id": self.agent_id, "dependency_wait": dependency_wait}
                return self.result
        ...
        parallelization_efficiency = (total_work_time / total_time) if total_time > 0 else 0
        avg_dependency_wait = statistics.mean([
            agent.result.get('dependency_wait', 0) for agent in agents 
            if hasattr(agent, 'result') and agent.result
        ])
        
        return [
            OrchestrationMetric(
                "dependency_chain_latency", total_time * 1000, "ms",
                "Total time to execute deep dependency chain", True
            ),
            OrchestrationMetric(
                "parallelization_efficiency", parallelization_efficiency, "ratio",
                "How well the framework parallelizes independent work", False
            ),
            OrchestrationMetric(
                "dependency_wait_time", avg_dependency_wait * 1000, "ms",
                "Average time agents wait for dependencies", True
            ),
        ]
    ...
    def wide_fanout_pattern(self) -> List[OrchestrationMetric]:
        ...
        async def run_fanout():
            ...
            # Start consumers
            consumer_tasks = [asyncio.create_task(consumer.run()) for consumer in consumers]
        ...
        consumer_start_spread = max_consumer_wait - min([c.wait_time for c in consumers])
        
        return [
            OrchestrationMetric(
                "fanout_total_time", total_time * 1000, "ms",
                "Total time for fanout pattern execution", True
            ),
            OrchestrationMetric(
                "consumer_avg_wait", avg_consumer_wait * 1000, "ms",
                "Average wait time for consumers", True
            ),
            OrchestrationMetric(
                "consumer_start_spread", consumer_start_spread * 1000, "ms",
                "Spread in consumer start times (lower = better coordination)", True
            ),
        ]
    ...
    def diamond_dependency_pattern(self) -> List[OrchestrationMetric]:
        ...
        class DiamondAgent(Agent):
            def __init__(self, name: str, dependencies: List['DiamondAgent'] = None):
        ...
        # Calculate metrics
        dag_efficiency = (0.06 / total_time) if total_time > 0 else 0  # 3 steps * 0.02s each
        max_dependency_wait = max([a.dependency_wait_time for a in agents])
        
        return [
            OrchestrationMetric(
                "diamond_dag_time", total_time * 1000, "ms",
                "Time to resolve and execute diamond DAG", True
            ),
            OrchestrationMetric(
                "dag_execution_efficiency", dag_efficiency, "ratio",
                "How efficiently the DAG is executed vs theoretical minimum", False
            ),
            OrchestrationMetric(
                "max_dependency_resolution_time", max_dependency_wait * 1000, "ms",
                "Maximum time any agent waits for dependencies", True
            ),
        ]
...
class ErrorHandlingBenchmarks:
    ...
    def cascading_failure_resilience(self) -> List[OrchestrationMetric]:
        ...
        class FailureProneAgent(Agent):
            ...
            async def run(self):
                ...
                if random.random() < self.failure_rate:
                    raise RuntimeError(f"Agent {self.name} failed on attempt {self.attempts}")
        ...
        return [
            OrchestrationMetric(
                "failure_handling_time", total_time * 1000, "ms",
                "Time to handle failures and retries", True
            ),
            OrchestrationMetric(
                "failure_recovery_rate", success_rate, "ratio",
                "Proportion of agents that eventually succeeded", False
            ),
            OrchestrationMetric(
                "retry_efficiency", avg_attempts, "attempts",
                "Average attempts needed per agent", True
            ),
        ]
    ...
    def partial_failure_isolation(self) -> List[OrchestrationMetric]:
        ...
        return [
            OrchestrationMetric(
                "isolation_execution_time", total_time * 1000, "ms",
                "Time to execute with isolated failures", True
            ),
            OrchestrationMetric(
                "failure_isolation_rate", isolation_rate, "ratio",
                "Rate of successful isolation from failures", False
            ),
            OrchestrationMetric(
                "partial_success_rate", failure_isolation_efficiency, "ratio",
                "Proportion of agents that completed despite other failures", False
            ),
        ]
...
class ResourceContentionBenchmarks:
    ...
    def concurrent_resource_pressure(self) -> List[OrchestrationMetric]:
        ...
        class ResourceIntensiveAgent(Agent):
            ...
            async def run(self):
                ...
                self.wait_time = time.perf_counter() - start_time - self.actual_work_time
                self.result = {"name": self.name, "computation": result}
                return self.result
        
        # Create agents with different resource requirements
        agents = [
            ResourceIntensiveAgent(f"light_{i}", 0.5) for i in range(10)
        ] + [
        ...
        contention_overhead = total_wait_time / total_work_time if total_work_time > 0 else 0
        resource_utilization = total_work_time / total_time if total_time > 0 else 0
        
        return [
            OrchestrationMetric(
                "resource_contention_time", total_time * 1000, "ms",
                "Total time under resource pressure", True
            ),
            OrchestrationMetric(
                "contention_overhead_ratio", contention_overhead, "ratio",
                "Ratio of wait time to work time", True
            ),
            OrchestrationMetric(
                "resource_utilization_efficiency", resource_utilization, "ratio",
                "How efficiently resources are utilized", False
            ),
        ]
    ...
    def memory_pressure_handling(self) -> List[OrchestrationMetric]:
        ...
        agents = [
            MemoryIntensiveAgent(f"small_{i}", 5) for i in range(8)
        ] + [
        ...
        memory_efficiency = total_allocated / (final_system_memory - initial_system_memory) if (final_system_memory - initial_system_memory) > 0 else 0
        
        return [
            OrchestrationMetric(
                "memory_pressure_time", total_time * 1000, "ms",
                "Time to execute under memory pressure", True
            ),
            OrchestrationMetric(
                "memory_allocation_efficiency", memory_efficiency, "ratio",
                "Efficiency of memory allocation and cleanup", False
            ),
            OrchestrationMetric(
                "peak_memory_usage", final_system_memory - initial_system_memory, "MB",
                "Peak additional memory used during execution", True
            ),
        ]
...
class ScalabilityBenchmarks:
    ...
    def horizontal_scaling_efficiency(self) -> List[OrchestrationMetric]:
        ...
        for agent_count in [10, 25, 50, 100]:
            ...
            asyncio.set_event_loop(loop)
            try:
                loop.run_until_complete(run_scaling_test())
            finally:
                loop.close()
        ...
        scaling_efficiency = final_throughput / baseline_throughput if baseline_throughput > 0 else 0
        
        # Calculate throughput consistency (lower std dev is better)
        throughputs = [r["throughput"] for r in scaling_results]
        throughput_variance = statistics.stdev(throughputs) / statistics.mean(throughputs) if len(throughputs) > 1 else 0
        
        return [
            OrchestrationMetric(
                "scaling_efficiency", scaling_efficiency, "ratio",
                "How well throughput scales with agent count", False
            ),
            OrchestrationMetric(
                "throughput_consistency", throughput_variance, "cv",
                "Coefficient of variation in throughput (lower is better)", True
            ),
            OrchestrationMetric(
                "max_throughput", max(throughputs), "agents/sec",
                "Maximum achieved throughput", False
            ),
        ]
    ...
    def coordination_overhead_scaling(self) -> List[OrchestrationMetric]:
        ...
        for coord_complexity in [5, 15, 30, 50]:  # Number of coordination points
            class CoordinationAgent(Agent):
                def __init__(self, agent_id: int, coordination_points: List[asyncio.Event]):
            ...
            agents = [CoordinationAgent(i, events) for i in range(20)]  # Fixed agent count
            ...
            asyncio.set_event_loop(loop)
            try:
                loop.run_until_complete(run_coordination_test())
            finally:
                loop.close()
            
            total_time = time.perf_counter() - start_time
            avg_coordination_time = statistics.mean([a.coordination_time for a in agents])
            coordination_overhead = avg_coordination_time / total_time if total_time > 0 else 0
        ...
        overhead_growth = coordination_results[-1]["overhead"] / coordination_results[0]["overhead"] if coordination_results[0]["overhead"] > 0 else 0
        max_coordination_time = max(r["avg_coord_time"] for r in coordination_results)
        
        return [
            OrchestrationMetric(
                "coordination_overhead_growth", overhead_growth, "ratio",
                "How coordination overhead grows with complexity", True
            ),
            OrchestrationMetric(
                "max_coordination_latency", max_coordination_time * 1000, "ms",
                "Maximum coordination latency observed", True
            ),
            OrchestrationMetric(
                "coordination_scalability", 1.0 / overhead_growth if overhead_growth > 0 else 1.0, "ratio",
                "Inverse of overhead growth (higher is better)", False
            ),
        ]
...
def print_orchestration_results(results: List[OrchestrationBenchmarkResult]):
    ...
    for result in results:
        print(f"\n🎯 {result.name}")
        print(f"   Framework: {result.framework} | Success Rate: {result.success_rate:.1%} | Iterations: {result.iterations}")
        print("-" * 80)
...
def save_orchestration_results(results: List[OrchestrationBenchmarkResult], filename: str):
...
def main():
    ...
    print("These benchmarks measure what actually matters for orchestration frameworks:")
