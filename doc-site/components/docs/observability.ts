...
provider = PrometheusMetricsProvider(MetricsConfig(namespace="puffinflow_app"))

# Define metrics once and reuse
request_counter = provider.counter(
    "workflow_requests_total", "Total workflow requests", ["workflow_type", "status"]
)
response_time = provider.histogram(
    "workflow_duration_seconds", "Workflow execution time", ["workflow_name"]
)
step_duration = provider.histogram(
    "ai_step_duration_seconds", "Step duration seconds", ["step_name", "model_type"]
)
active_workflows = provider.gauge(
    "active_workflows_current", "Currently active workflows"
)
queue_depth = provider.gauge(
    "workflow_queue_depth", "Workflow queue depth", ["priority"]
)
...
async def ai_processing_task(context):
    ...
    try:
        ...
        request_counter.record(1, workflow_type="ai_processing", status="started")
        active_workflows.record(1)
        ...
        for step in processing_steps:
            ...
            step_duration.record(
                step_duration, step_name=step['name'], model_type=model_type
            )
        
        # Calculate total processing time
        total_duration = time.time() - start_time
        response_time.record(total_duration, workflow_name="ai_processing")
        ...
        # Success metrics
        request_counter.record(1, workflow_type="ai_processing", status="completed")
        
        print(f"✅ AI processing completed in {total_duration:.2f}s")
        
    except Exception as e:
        # Error metrics
        request_counter.record(1, workflow_type="ai_processing", status="failed")
        print(f"❌ AI processing failed: {e}")
        raise
    
    finally:
        active_workflows.record(0)
...
async def data_pipeline_task(context):
    ...
    try:
        ...
        for i, batch_size in enumerate(batch_sizes):
            ...
            ).record(batch_duration, batch_size_range=get_size_range(batch_size))
            ...
            ).record(throughput, pipeline_stage="processing")
        
        total_duration = time.time() - start_time
        
        # Record pipeline completion metrics
        provider.histogram("pipeline_total_duration_seconds").record(total_duration)
        
        provider.counter(
            "pipeline_records_total", labels=["status"]
        ).record(pipeline_metrics["records_processed"], status="processed")
        ...
        provider.counter("pipeline_errors_total", labels=["error_type"]).record(
